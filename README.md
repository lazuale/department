...
11. Отдельный `Запрос` закрывается только после фактического получения требуемого документа или материала либо после явной отмены потребности.
12. Количество закрытых карточек не является рейтингом сотрудников.
13. Идея не является обязательством, пока не принято решение о реализации.
