...
Поддерживает устойчивые правила процесса:
...
Он не проверяет вручную каждую карточку.
...
1. критичные работы;
2. обычные с ближайшим сроком;
3. при одинаковом сроке — более старые;
4. остальные.
...
После решения `делаем` создаётся обязательная работа или проект.

До этого идея не конкурирует за срок с обязательной работой.

## 19. Как меняется сама методика
