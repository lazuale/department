...
Этот файл — единственный источник общих правил. Остальные документы показывают их применение и не вводят отдельные нормы.
...
| Тип `Работа` | обычный обязательный результат, который не является отдельным `Запросом` или `Регламентом` |
...
В тексте слово «работа» без кодового оформления означает обязательную работу в общем смысле. `Работа` — конкретный тип карточки.

**Тип и категория — не одно и то же.** Тип отвечает на вопрос «что это за объект», категория — «какой процесс его породил».
...
**Зачем:** иначе один реальный результат распадается на набор технических действий и перестаёт нормально контролироваться.
...
**Зачем:** исходную постановку нельзя тихо переписать задним числом и скрыть ошибку, просрочку или новый объём.
...
| у исполнителя много другой работы | `Очередь`, не `Ожидание` |
| пока не дошли руки | `Очередь` |
| переключились на критичную работу | прежняя работа → `Очередь`, если её никто не продолжает |
| не понимаем, что делать дальше | уточнение / эскалация, не фиктивное `Ожидание` |
| результат достигнут | `Готово` |

**Зачем различать `Очередь` и `В работе`:** иначе невозможно увидеть реальную загрузку.

**Зачем ограничивать `Ожидание`:** оно должно показывать внешнюю блокировку, а не скрывать отложенную работу.
...
**Зачем:** перенос срока не должен стирать факт просрочки.
...
**Зачем:** `отправили` не должно превращаться в `сделали`.
...
После решения `делаем` создаётся обязательная работа. Её масштаб определяется по разделу 15.

**Зачем:** необязательные улучшения не должны конкурировать со сроками обязательной работы.
