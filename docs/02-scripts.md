...
**Ситуация:** пришло письмо, документ, сообщение или другое основание.
...
До передачи рабочего периода входящие до установленного локальным правилом контрольного времени должны получить определённую судьбу. Более поздние явно переходят следующему дежурному.

---

## PS-02. Создание обычной работы

**Ситуация:** появился новый самостоятельно контролируемый результат.
...
Если внешнего срока нет, используется внутренний срок процесса. Если он ещё не определён, автор обращается к ответственному за направление и не придумывает дату произвольно.
...
**Ситуация:** исполнитель может начать следующую работу.
...
Если порядок нескольких критичных работ нельзя определить по сроку без ущерба для других обязательств, решение принимает руководитель.
...
**Ситуация:** после создания выяснилось, что срок, категория, исполнитель, приоритет, название или описание указаны неверно либо условия реально изменились.
...
**Ситуация:** продолжать сейчас невозможно из-за внешней причины.
...
**Ситуация:** подразделению по собственной инициативе нужен важный документ, материал или подтверждение, получение которого нельзя потерять после отправки.
...
- срок получения;
- владельца либо контролируемую групповую очередь до фактического взятия.

До отправки `Запрос` при необходимости может находиться в групповой очереди. Как только сотрудник берёт его на подготовку, он назначает себя исполнителем.

После отправки запрос переводится `В работе → Ожидание` и остаётся открытым **за конкретным владельцем сопровождения**. Возвращать отправленный запрос в безличную групповую очередь нельзя.
...
Ответственный за направление устанавливает горизонт предварительного создания работ для процесса и следит, чтобы следующий период появился в системе заранее.

Если очередная карточка не создана вовремя:

1. создать пропущенную работу;
2. проверить соседние будущие периоды;
3. ответственный за направление разбирает причину и при необходимости меняет локальное правило процесса.
...
- **выборочная проверка** — небольшая обычная выборка завершённых работ с периодичностью, установленной для процесса;
...
2. создать обязательную работу;
3. определить её масштаб по обычному правилу: одна карточка, иерархия работ или отдельный проект;
4. связать новую работу с исходной идеей;
5. после реализации проверить, изменился ли исходный процесс.
